...
public class JazzyViewPager extends ViewPager {
    ...
    public JazzyViewPager(Context context, AttributeSet attrs) {
        ...
        String[] transitions = getResources().getStringArray(R.styleable.JazzyViewPager_style);
...
//}
    ...
    protected void animateScroll(int position, float positionOffset) {
        if (mState != State.IDLE) {
            ...
            this.setRotationY(mState == State.GOING_RIGHT ? mRot : -mRot);
            this.setPivotX(getMeasuredWidth() * 0.5f);
            this.setPivotY(getMeasuredHeight() * 0.5f);
        }
    }
    ...
    protected void animateRevertStack(View left, View right, float positionOffset, int positionOffsetPixels) {
        if (mState != State.IDLE) {
            if (right != null) {
                /*manageLayer(right, true);
                mScale = (1 - SCALE_MAX) * positionOffset + SCALE_MAX;
            ...
            if (left != null) {
                ...
                mScale = 1 - (1 - SCALE_MAX) * positionOffset;
                mTrans = positionOffsetPixels;
                left.setScaleX(mScale);
                left.setScaleY(mScale);
                left.setTranslationX(mTrans);
...
//		mRight = getChildAt(position+1);
        ...
        switch (mEffect) {
            ...
            case Accordion:
                ...
                break;
            case RevertStack:
                animateRevertStack(mLeft, mRight, effectOffset, positionOffsetPixels);
                break;
        }
